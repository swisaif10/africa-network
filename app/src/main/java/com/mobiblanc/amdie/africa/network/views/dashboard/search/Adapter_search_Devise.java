...
import com.mobiblanc.amdie.africa.network.models.search.init_montoring.Devise;
...
public class Adapter_search_Devise extends RecyclerView.Adapter<Adapter_search_Devise.ViewHolder> {

    private final Context context;
    private final Devise data;
    private final CheckedLisner checkedLisner;


    public Adapter_search_Devise(Context context, Devise data , CheckedLisner checkedLisner ) {
    ...
    void enable_check(Boolean enable){
        ...
        checkedLisner.Checked_Devise(nbr_check()>0,getList_id_String(),getList_name_String());
    }
    int nbr_check(){
        int nn=0;
        for (int i=0 ;i< data.getObjetReferenceValues().size();i++){
